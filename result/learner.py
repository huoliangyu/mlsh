...
class Learner:
    ...
    def updateMasterPolicy(self, seg,is_test = False):
        ...
        lrlocal = (seg["ep_lens"], seg["ep_rets"],seg["ep_ssims"],seg["ep_jitters"],seg["ep_rebuffings"],seg["ep_freezes"],seg["ep_creals"],seg["ep_buffers"])
        listoflrpairs = MPI.COMM_WORLD.allgather(lrlocal) # list of tuples
        lens, rews,ssims,jitters,rebuffings,freezes,creals,buffers = map(flatten_lists, zip(*listoflrpairs))
        logger.record_tabular("EpRewMean", np.mean(rews))
        sub_rate = self.sub_rate(num_of_sub)

        return np.mean(rews), np.mean(seg["ep_rets"]),sub_rate, np.mean(ssims), np.mean(jitters), np.mean(rebuffings), np.mean(freezes),np.mean(creals),np.mean(buffers)
    ...
    def add_master_record(self):
        ...
        for x in range(num_task):
            with open(self.logdir+"/MRL_task_{}.txt".format(x),"w") as f:
                f.write("iter\treward\tssim\tjitter\trebuffing\tfreeze\tcreal\tbuffer\tsubrate\n")
        # add to summary
    ...
    def add_total_info(self,it,real_goal,rew_total,rew_cur,sub_rate,test_rew,test_ssim,test_jitter,test_rebuffing,test_freeze,test_creal,test_buffer,test_subrate):
        '''
        ...
        with open(self.logdir+"/MRL_task_{}.txt".format(real_goal),"a") as f:
            f.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(it,test_rew,test_ssim,test_jitter,test_rebuffing,test_freeze,test_creal,test_buffer,test_subrate))
