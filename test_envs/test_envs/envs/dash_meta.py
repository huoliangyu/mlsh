...
is_multi_Dt = True
is_randomsize = True
# is_reset_random_goal = False
markov_level = 2
# is_quantization_download_time = True
is_symmetry_Dt_for_qt = True #true is normal
is_use_obs_in_state = True
is_use_last_downloadtime = False
is_gauss_random = False
is_use_DQN = False
if is_use_DQN:
    is_reset_random_goal = True
    is_quantization_download_time = False
else:
    is_reset_random_goal = False
    is_quantization_download_time = True
...
# c_list = [0.4, 0.75, 1.5, 2.5, 3.5, 4.5, 5.75, 7.25, 9, 12.5]
# goal_list=[[1,2,50],[-1,-2,-50]] #default [1,2,50]
# goal_list=[[1,0,0],[0,2,0],[0,0,50]]
# goal_list = [[1,2,50],[1,0.4,2],[1,10,10]]
# goal_list = [[1,2,50],[1,6,30],[1,0.6,10]]
# goal_list = [[0,0,1]]

def generate_goal(index_num):
    goal_list = []
    goals = [[1,10,50],[1,2,250],[1,5,10],[1,6,30],[1,0.6,10]]
    # goals = [[1,2,50],[1,0.4,2],[1,10,10],[1,6,30],[1,0.6,10]]
    for x in range(index_num):
        goal_list.append(goals[x])

    return goal_list
index_num = 3
# goal_list = [[1,2,50],[1,6,30],[1,0.6,10]]
goal_list = generate_goal(index_num)
print ("num-task is {}".format(index_num))
print ("current_goal is {}".format(goal_list))
...
class DashMeta(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Discrete(len(qt_list))
    ...
    def randomizeCorrect(self):
        if is_randomsize:
            self.realgoal = self.np_random.randint(0,len(goal_list))
        else:
            self.realgoal=0
        if is_gauss_random:
            alpha_list = [1,1,1]
            beta_list = np.random.normal(2,1,3)
            beta_list[beta_list<0.1] = 0.1
            beta_list = beta_list.tolist()
            gamma_list = np.random.normal(10,5,3)
            gamma_list[gamma_list<0.1] = 0.1
            gamma_list = gamma_list.tolist()
            global goal_list
            for x in range (len(goal_list)):
                goal_list[x] = [alpha_list[x],beta_list[x],gamma_list[x]]
    ...
    def step(self, action):
        if is_symmetry_Dt_for_qt is False and is_multi_Dt:
            if D_list.index(self.Dt) !=0 and action==0:
                action = 1
        is_overflow=  False
        ...
        c_real_record = self.c_real
        if is_quantization_download_time:
            if tao>10:
                tao = 10
        old_phi = self.phi
        self.phi = max(0,tao-self.Bt)
        cur_freeze = 0
        if self.T +max(0,self.Bt-tao) >self.Bmax:
            is_overflow = True
            self.phi = old_phi
            ...
            obs = self.obs()
            # print ("overflow--Bt:{} tao:{}".format(self.Bt,tao))
            final_state = [None,None,None,{"overflow":is_overflow}]
            ...
            if tao>self.Bt:
                ...
                cur_freeze = 1
            info = "tao:{} Bt:{} c:{} size:{} qt:{}".format(tao,self.Bt,self.c_real,f,qt_list.index(self.qt))
            ...
            # r_rebuffing = self.phi*1.0/self.max_rebuffing_time
            r_rebuffing  = self.phi*1.0/(self.phi+self.T)*1.1
            # print ("self.phi is {}".format(self.phi))
            # if r_rebuffing>0:
            #     print ("--{}-----freeze----{}---{}".format(self.cur_step,r_rebuffing,info))
            r_future = max(0,self.Bth - self.Bt)**2
            
            reward = self.alpha*r_ssim - self.beta*r_jitter - self.gamma* r_rebuffing -self.eps *r_future
            self.state = self.get_state()
            obs = self.obs()
            ...
            if self.cur_step>= self.episode_num:
                self.terminal = 1
            final_state = [obs,reward,self.terminal,{"overflow":is_overflow,"total_rebuf_time":self.rebuffing_time,"total_rebuf_num":self.rebuffing_num,"ssim":r_ssim,"jitter":r_jitter,"rebuf_time":r_rebuffing,"freeze":cur_freeze,"c_real":c_real_record,"buffer":self.Bt}]
            if self.terminal==1:
                self.reset()
        return final_state

    def obs(self):
        if is_use_obs_in_state:
    ...
    def reset(self):
        ...
        self.D_queue = self.create_D(self.episode_num*2)
        self.Dt = self.D_queue.get()
        self.cur_step = 0
        self.terminal = 0
        self.rebuffing_time = 0
        self.rebuffing_num = 0
        if is_reset_random_goal:
            self.randomizeCorrect()
        return self.obs()
    ...
    def get_state(self):
        if is_normalize_obs:
            ...
            if is_use_last_downloadtime:
                return [self.qt] + state_c_list +[self.phi/self.max_rebuffing_time,self.last_time/self.max_rebuffing_time,self.Bt*1.0/self.Bmax]
            else:
                return [self.qt] + state_c_list +[self.phi/self.max_rebuffing_time,self.Bt*1.0/self.Bmax]
        else:
            state_c_list = [x for x in self.c_estimate]
            if is_use_last_downloadtime:
                return [self.qt] + state_c_list +[self.phi,self.last_time,self.Bt]
            else:
    ...
    def create_D(self,queue_length):
        ...
        while True:
            # get one D
            if is_multi_Dt:
                D_index = np.random.randint(0,len(D_list))
            else:
                D_index = 3
    ...
    def F (self,qt,Dt):
        ...
        return np.array(np.power(10,result)*self.fmax)
    def Markov(self,c_cur):
        ...
        if markov_level == 2:
            ...
            raise IOError("error Markov Level--{}".format(markov_level))
    ...
    def generate_f_dict(self,qt_list,D_list):
        ...
        for qt_index,qt in enumerate(qt_list):
            for Dt_index,Dt in enumerate(D_list):
                ...
                # print ("({},{})------qt:{} Dt:{} size:{}".format(qt_index,Dt_index,qt,Dt,self.F_dict[qt_index][Dt_index]))
        self.max_rebuffing_time = np.array(np.power(10,self.F_dict.max())*self.fmax)
        print ("generate F_dict done,self.max_rebuffing_time is {}".format(self.max_rebuffing_time))
    def GetF(self,qt,Dt):
        x=sympy.Symbol('x')
        zero =- qt
